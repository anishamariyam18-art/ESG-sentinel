...
def get_evidence_indexes() -> tuple[EvidenceIndexer, LexicalIndex]:
    ...
    if _indexer is None or _lexical_index is None:
        ...
        if existing:
            ...
            logger.info("evidence_indexes_rebuilt_from_repository", extra={"count": len(existing)})
...
def get_evidence_retriever() -> EvidenceRetriever:
    ...
    if _retriever is None:
        ...
        _retriever = EvidenceRetriever(repository=get_evidence_repository(), indexer=indexer, lexical_index=lexical_index)
    return _retriever
...
def get_result_store() -> PipelineResultStore:
    ...
    if _result_store is None:
        _result_store = PipelineResultStore(get_settings().outputs_dir)
...
def get_pipeline_orchestrator() -> PipelineOrchestrator:
    ...
    if _orchestrator is None:
        ...
        _orchestrator = PipelineOrchestrator(
            repository=get_evidence_repository(), indexer=indexer, lexical_index=lexical_index,
            retriever=get_evidence_retriever(), llm_manager=get_pipeline_llm_manager(),
        )
...
def reset_dependencies_for_testing() -> None:
    ...
    global _repository, _indexer, _lexical_index, _retriever, _result_store, _orchestrator
    _repository = None
    _indexer = None
    _lexical_index = None
    _retriever = None
